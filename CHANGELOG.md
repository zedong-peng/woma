...
- Added one stable Skill root per Environment shared by every target Agent, so an ordinary Skill installed through Codex, Claude Code, Pi, or Qoder CLI is immediately visible to all targets without a second command; legacy per-Agent layouts merge transactionally with conflict checks, hidden state remains excluded from inventory, and other Environments remain isolated.

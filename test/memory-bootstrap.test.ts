...
test("Memory bootstrap points to the normally activated Skill and preserves user instructions", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-"));
  ...
  try {
    ...
    assert.match(activated, /installed `woma-project-memory` Skill/);
...
test("Pi discovers Project Memory through AGENTS.md", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-pi-"));
  try {
...
test("Qoder discovers Project Memory through AGENTS.md", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-qoder-"));
  try {
...
test("Memory bootstrap never removes another target's stable discovery pointer", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-switch-"));
  const agentsPath = path.join(root, "AGENTS.md");
  ...
  try {
    ...
    assert.match(await readFile(agentsPath, "utf8"), /Woma Project Memory/);
    assert.match(await readFile(claudePath, "utf8"), /installed `woma-project-memory` Skill/);
...
test("Memory bootstrap preserves instruction symlinks and their targets", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-symlink-"));
  ...
  try {
    ...
    assert.match(await readFile(target, "utf8"), /Shared instructions[\s\S]*Woma Project Memory/);
...
test("Memory bootstrap is absent when the ordinary Memory package is not active", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "woma-memory-bootstrap-opt-out-"));
  try {

...
- Stable per-Environment Agent homes and views may contain credentials, provider configuration, sessions, databases, and other private state. Original Agent configuration seeds credentials and provider settings when an Environment is initialized; `base` also adopts missing opaque entries from the original Agent homes once so existing sessions remain available. Adoption links entries without copying symlink targets or overwriting stable entries, so writes through an adopted path may update the original home. Subsequent configuration edits are Environment-specific. Harness creates homes and views with user-only permissions; protect `$HARNESS_HOME` and the original Agent configuration directories and never publish them.
- After the one-time `base` adoption, Harness never copies or transfers unknown Agent state between Environment homes or view generations. In particular, SQLite main, WAL, and SHM files remain in place rather than being snapshotted and are excluded from Package locks and bundles.

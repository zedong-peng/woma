...
- Explicit `harness migrate sessions` copies only documented session and history paths into the selected stable Agent home. It rejects symbolic links and special files, verifies that the source did not change while the temporary snapshot was built, and fails before publication on different same-path target content. The resulting ordinary files are private Environment state, are not exportable Package content, and no longer depend on the original home.

...
```bash
woma env list
woma create --name <name> [--target codex|claude|pi|qoder|opencode|both|all|<comma-separated-list>]
woma rename --name <environment> <new-name>
woma env remove <name>
```

`base` is initialized automatically and cannot be explicitly created or removed. It starts with clean managed Agent configuration and no root Packages. For backward compatibility, `base`, `both`, and an omitted `--target` select Codex and Claude. Use `--target pi`, `--target qoder`, `--target opencode`, a comma-separated combination such as `codex,opencode`, or `--target all` explicitly. Other new Environments also contain no root Packages until they are installed explicitly.

`env create <name>` remains a compatibility spelling for `create --name <name>`. `rename` refuses `base`, an active source Environment, and every existing destination. It moves the complete Environment directory, including Agent-owned credentials, sessions, databases, and other opaque state stored under that prefix, then validates the renamed managed view before succeeding. Ambient Agent state outside the prefix, including OpenCode's native data locations, is not moved. If the source was the initialized shell default, that default follows the destination name. Removing the initialized default resets it to `base`. These lifecycle operations share the initialization lock, and a failed Environment mutation restores the previous default references.

On the first successful implicit creation of `base` outside init, Woma uses metadata checks on only the documented Codex/Claude Skill and session migration paths. If a real supported file or directory exists, it writes one notice to stderr. It does not enumerate children, read contents, follow symlinks, print private names or paths, or import any state into `base`. The notice points to `woma init`, whose separate one-time state remains eligible even when `base` already exists. Shell-hook and JSON stdout remain machine-readable.

`woma init` deliberately goes one step beyond Conda's shell-only initialization because changing Agent homes without a compatibility import would hide an established Codex setup. It copies only supported files into a new Environment and never registers, mutates, or links the arbitrary original home as a Woma Environment.
...
Recognition is intentionally non-recursive. A repository root containing several nested Package directories is a collection, not an installable Package; install one child directory explicitly. When an unsupported Git repository root contains multiple installable direct children, Woma lists those Package candidates in stable order and suggests an explicit `woma install <source> --subdir <candidate>` command. It does not search beyond direct children. Woma does not invoke an Agent, infer MCP servers, Hooks, dependencies, or entrypoints, or write a generated manifest into the source.
...
For an implicit Package, Woma parses each Skill's `name` and `description` from YAML frontmatter, copies the selected self-contained Skill directories into temporary staging, and generates a deterministic `woma.yaml` supporting Codex, Claude, Pi, Qoder, and OpenCode. A standalone Package uses the Skill name as its Package name. A multi-Skill Package derives its name from the selected local or Git source directory. Its informational version is `0.0.0+local.<content>` for local content or `0.0.0+git.<commit>` for Git. The ordinary manifest, symlink, identity, ownership, cache, and Environment transaction validation then applies.

Installation recursively resolves dependencies, validates Package and Skill identities and SemVer constraints, rejects cycles and source conflicts, and publishes one complete global Agent view generation atomically. Package Store entries are read-only after publication. Stable-home MCP and Hook files are reconciled in the same transaction while unknown Agent fields remain in place. Every project using that Environment observes the new view without reactivation; already-running Agent processes may need a restart to rediscover Skills.
...
`activate` defaults to `base`. With the recommended shell hook installed, it validates the selected Environment and selects each supported Environment's stable Agent home in the parent shell. Unsupported Agents remain on their original configuration homes. Codex uses `CODEX_HOME`, Claude uses `CLAUDE_CONFIG_DIR`, Pi uses `PI_CODING_AGENT_DIR`, and Qoder uses `QODER_CONFIG_DIR`. Environment selection belongs only to the shell and is never recorded in the project. Activation does not migrate layouts or edit project files.

Environment recipes and view metadata do not carry schema-version fields. Their strict current structure is the format, and unexpected fields fail validation rather than selecting a compatibility path. Explicit `woma migrate` commands import Agent-owned data into the current model; they are not Woma schema migrations.
...
Authentication and provider configuration also belong to the selected Environment where the Agent exposes a dedicated root. For Codex, log in after creating the Environment and edit `$CODEX_HOME/config.toml`; Codex owns the ordinary `$CODEX_HOME/auth.json` file, which Woma does not seed from the original home or include in managed views. For Claude endpoint and API-key settings, edit `$CLAUDE_CONFIG_DIR/settings.json`; Claude OAuth login may additionally create `$CLAUDE_CONFIG_DIR/.credentials.json`. Managed provider configuration under the original `~/.codex` and managed configuration and credentials under `~/.claude` seed a new Environment only and do not update existing Environments. Codex `config.toml` and `hooks.json`, Claude `settings.json` and `.claude.json`, and Qoder `settings.json` are regular stable-home files. Woma reconciles only its previous MCP/Hook values, preserves unknown and Agent-owned fields, and fails on changed Woma-owned values. For Pi, use `/login`, `/model`, or files under `$PI_CODING_AGENT_DIR`; Pi owns settings, credentials, models, Pi Packages, and sessions in the stable Environment home. A new Pi Environment does not copy the original `~/.pi/agent` state. For Qoder, log in through `qodercli` in the activated Environment. For OpenCode, Woma sets `OPENCODE_CONFIG` to `home/opencode/opencode.json` and `OPENCODE_CONFIG_DIR` to `home/opencode`; the file contains only Package MCP servers and the directory supplies shared Skills. Woma does not override `XDG_*`, so OpenCode provider authentication, MCP OAuth tokens, sessions, and caches continue to use their native global locations and remain outside Woma's managed Environment view.

Each shell selects its own Environment through `WOMA_ENV`, so multiple shells can run Agents from different stable homes concurrently. Credentials, provider configuration, opaque runtime state, and Codex system Skills remain isolated when the Agent exposes a dedicated selected home. OpenCode's Woma-managed Skills and MCP overlay are isolated, but its ambient native data is not. Woma does not add a separate secret store: secret values remain in Agent-owned locations or the shell environment and are excluded from Package manifests, locks, and Environment bundles.
...
`deactivate` leaves Woma Environment management in the current shell: it clears `WOMA_ENV` and restores the `CODEX_HOME`, `CLAUDE_CONFIG_DIR`, `PI_CODING_AGENT_DIR`, `QODER_CONFIG_DIR`, `OPENCODE_CONFIG`, and `OPENCODE_CONFIG_DIR` values captured before the shell hook selected an Environment. Use `woma activate base` when you want to switch explicitly to Woma's `base` Environment. Woma does not currently associate Agent session IDs with Environments; activate the intended Environment before resuming an existing session.
...
`info` also reports whether each supported native Agent CLI is available on the current `PATH` and shows the resolved executable path when found. It checks `codex`, `claude`, `pi`, `qodercli`, and `opencode` without launching them. `info --json` is the stable machine-readable Environment interface; its `agentClis` mapping contains each Agent's command, availability, and resolved path in addition to the configured project directory, selected Environment, Packages, Skills, entrypoints, `environmentSkills`, `environmentSkillIssues`, and `environmentMcpServers`. Each external Environment Skill has `platform: "environment"` and a `platforms` array containing every target that sees the shared root. `environmentMcpServers` reports read-only native MCP discoveries with `origin: "external"`; the initial implementation inspects OpenCode global and explicitly selected config files without copying or adopting them. It replaces the redundant user-facing `current` command.

`inspect` applies the same source recognition and normalization rules as installation. `doctor` validates dependency locks, the exact Package-managed Skill visibility closure, the global view, platform support, executable and environment requirements, and selected targets. It never reads Memory data. For each Environment target it reports the native Agent CLI path or warns when that CLI is not available; a missing Agent CLI does not make an otherwise portable Environment corrupt. It also reports valid Environment-local Skills, warns about invalid `SKILL.md` metadata, and fails on names that conflict with Woma-managed Skills. Hidden children such as `.system` are excluded from this inventory.
...
On its first successful run, init also creates a clean `base` and records a one-time bootstrap decision in `$WOMA_HOME/initialization.json`. It detects only regular original Codex `config.toml` and `hooks.json` files plus ordinary non-hidden Skills. If any exist, init creates the Codex-only `codex` Environment, seeds those two configuration files, snapshots the Skills as Woma Packages, and writes `codex` to `$WOMA_HOME/default-environment`; otherwise it writes `base`. It never copies Codex authentication, system Skills, Plugins, sessions, history, caches, databases, telemetry, native memory, or unknown state. It does not modify the original home or manage the Codex executable.

Initialization is automatic and non-interactive. It first validates the shell profile, Codex TOML and JSON configuration, and Skill import plan. A conflicting pre-existing `codex` Environment fails with instructions to rename or remove it. Versioned pending state and an atomically reserved, matching imported-Environment marker allow retry after interruption without adopting an unrelated Environment. The `codex` Environment lock covers creation, Skill installation, completion, and rollback. Repeated init keeps the completed choice and does not import later changes. Renaming that default follows the new Environment name; removing it resets the default to `base`, with both changes serialized against init and rolled back on ordinary failure. A later init replaces an incomplete recorded non-base default with `base`. `--dry-run` performs the same input validation and prints the complete file plan without creating `$WOMA_HOME`, Environments, Package snapshots, or profile changes. `--reverse` removes the managed profile block and static hook while preserving user-owned profile content, modes, symbolic links, Environments, the initialized default, and the one-time decision.

The sourced hook saves the original Codex, Claude, Pi, Qoder, and OpenCode configuration variables and performs bounded path checks before exporting an existing Environment's view. When `WOMA_ENV` is unset, it reads the initialized default and falls back to `base`. A stale or missing selected Environment falls back to an existing usable `base` with a warning. If `base` is also unavailable, it restores all original Agent homes and leaves no Woma Environment selected. It restores the original variables for each unsupported target and shows `(woma:<environment>)` only when selection succeeds. Only a successful top-level `activate` or `deactivate` updates the parent shell; help and unrelated commands are inert. The static hook does not launch Node, mutate Woma state, acquire locks, or proxy `codex`, `claude`, `pi`, `qodercli`, or `opencode`.

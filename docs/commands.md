...
`base` is initialized automatically and cannot be explicitly created or removed. It starts with clean managed Agent configuration and no root Packages. For backward compatibility, `base`, `both`, and an omitted `--target` select Codex and Claude. Use `--target pi`, `--target qoder`, a comma-separated combination such as `codex,pi`, or `--target all` explicitly. Other new Environments also contain no root Packages until they are installed explicitly.

`env create <name>` remains a compatibility spelling for `create --name <name>`. `rename` refuses `base`, an active source Environment, and every existing destination. It moves the complete Environment directory, including Agent-owned credentials, sessions, databases, and other opaque state, then validates the renamed managed view before succeeding. If the source was the initialized shell default, that default follows the destination name. Removing the initialized default resets it to `base`. These lifecycle operations share the initialization lock, and a failed Environment mutation restores the previous default references.
...
The sourced hook saves the original Codex, Claude, Pi, and Qoder configuration roots and performs bounded path checks before exporting an existing Environment's view. When `WOMA_ENV` is unset, it reads the initialized default and falls back to `base`. A stale or missing selected Environment falls back to an existing usable `base` with a warning. If `base` is also unavailable, it restores all original Agent homes and leaves no Woma Environment selected. It restores the original root for each unsupported target and shows `(woma:<environment>)` only when selection succeeds. Only a successful top-level `activate` or `deactivate` updates the parent shell; help and unrelated commands are inert. The static hook does not launch Node, mutate Woma state, acquire locks, or proxy `codex`, `claude`, `pi`, or `qodercli`.

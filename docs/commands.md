...
For a Package below a Git repository root, pass `--subdir` with a normalized repository-relative path. The selected directory may be a standalone `SKILL.md` or a conventional directory containing every direct `skills/*/SKILL.md`; the latter becomes one Package exposing all of those Skills independently. Without `--subdir`, the same rules apply at the repository root. `--commit` accepts an immutable full commit SHA and defaults to the latest commit on the repository's default branch. The Environment lock keeps the source repository, subdirectory, full commit, and normalized content integrity as distinct provenance. `harness sync` uses the locked commit after the default branch moves, and the original clone is not required after installation.
...
Authentication and provider configuration belong to the selected Environment. For Codex, edit `$CODEX_HOME/auth.json` and `$CODEX_HOME/config.toml`. Codex owns `$CODEX_HOME/skills/.system`, including whether that path is absent, a directory, or another Codex-managed representation; Harness never adopts that hidden runtime path. Ordinary Skills that Codex adds to the stable `skills` directory remain runtime state until the Environment is explicitly synchronized. For Claude endpoint and API-key settings, edit `$CLAUDE_CONFIG_DIR/settings.json`; Claude OAuth login may additionally create `$CLAUDE_CONFIG_DIR/.credentials.json`. Managed configuration files under the original `~/.codex` and `~/.claude` homes seed a new Environment only and do not update existing Environments. For Pi, use `/login`, `/model`, or files under `$PI_CODING_AGENT_DIR`. Harness manages only its `skills` link; Pi owns settings, credentials, models, Pi Packages, and sessions in the stable Environment home. A new Pi Environment does not copy the original `~/.pi/agent` state.
...
`inspect` applies the same source recognition and normalization rules as installation. `sync` first restores missing or corrupt content-addressed Package entries from exact lock sources, repeats the same normalization for implicit Packages, and rebuilds the global view. For each selected Codex Environment, it then discovers ordinary, unmanaged direct children of `$CODEX_HOME/skills` that contain `SKILL.md`, snapshots each as a content-addressed Package, and installs those Packages through the normal Environment transaction. The runtime directory becomes a managed Skill link only after the lock and complete view publish successfully; an ordinary failure restores the original runtime entry.

This is an explicit synchronization boundary, not a background daemon and not an Agent proxy. Hidden entries such as `.system`, entries already owned by locked Packages, and state in other Environments are excluded. When new runtime Skills are present, the same running-Agent warning used by migration asks the user to stop active Agents before capture; a non-interactive command fails closed when an Agent is detected, and Harness checks the source hash again before staging it. Restart already-running Agents after successful adoption so they rediscover the managed Skill. `sync` can also repair `base` when its recipe and lock remain parseable even if its Package cache or view is damaged. `doctor` validates dependency locks, the exact Skill visibility closure, the global view, platform support, executable and environment requirements, selected targets, and Memory discovery instructions.

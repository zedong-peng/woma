...
Codex may also install an ordinary Skill while running inside an already selected Environment. Explicit `harness sync` is the reconciliation point for that Environment-local state: after repairing the existing lock, it snapshots each unmanaged, non-hidden direct Skill child as a migration Package and installs the new roots with the ordinary transaction. Runtime entries are staged only for publication and restored on an ordinary failure. The resulting Skill is visible in the Environment lock and `harness list`; other Environments are unchanged. Hidden paths such as Codex-owned `.system` are never adopted. Harness does not watch the directory continuously or proxy the Codex process.

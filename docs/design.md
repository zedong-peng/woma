...
Package Store entries have their write bits removed before publication, and Skill directories in a view are symbolic links into those read-only entries. Every update builds a complete `.view.gen-<id>` directory and atomically replaces the stable `view` symlink, so direct Agent readers never observe mixed Skill, MCP, and Hook generations. Non-Harness authentication and session paths are linked through a stable shared runtime root, seeded from the user's original Agent configuration roots. Known first-use paths are linked before they exist, arbitrary runtime files are adopted before an old generation is retired, and missing links are repaired rather than interpreted as permission to delete credentials. Claude non-`mcpServers` fields use one authoritative shared snapshot; switching replaces that snapshot exactly, including field deletion, while the target Environment retains only its own and user-baseline MCP definitions.

The shell hook saves the original Agent configuration roots. It validates that the selected Environment and complete view exist before changing the parent shell, falls back to `base` for a stale inherited selection, and parses only a real top-level `activate` or `deactivate` command. Help requests and unrelated arguments never change Environment state. For unsupported targets it restores the original Agent configuration root. It never proxies `codex` or `claude`.

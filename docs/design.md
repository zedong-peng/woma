...
| Explicit Skill migration snapshots | User-global | `$WOMA_HOME/migrations/skills/<skill-name>/<content-hash>/` |
| Mutation locks | User-global | `$WOMA_HOME/locks/` |
| Environment selection | Current shell | `WOMA_ENV` (defaults to `base`) |
...
Each Environment has stable target Agent homes that are never generation-swapped and one real `home/skills` directory shared by all targets. Every target home's conventional `skills` path links to that directory. Woma-owned Skill entries inside it link through the atomic `view/skills` projection, while non-hidden ordinary additions are Environment-local Skills with external origin. Other Environment-specific configuration still links through target views: Codex `config.toml` and `hooks.json`; Claude `.credentials.json` and `settings.json`; and Qoder `settings.json`. Codex owns the ordinary `auth.json` in its stable Environment home; Woma does not seed it from the original home or include it in managed views. A legacy Woma-managed Codex credential link is transactionally detached into an ordinary file during the next view repair or update by copying its bytes without parsing them, with rollback on publication failure. Existing layouts with separate projected Agent Skill roots upgrade on base repair, named Environment activation, install, or uninstall. The upgrade moves each unique non-Woma entry intact into the shared root, deduplicates same-name entries only after verifying that their complete trees are equivalent, and fails before mutation when copies differ or Package ownership conflicts. Codex provider settings and Claude credentials and provider settings are seeded from the original Agent home only when an Environment is first built, then inherited from the current generation so user edits through `$CODEX_HOME` or `$CLAUDE_CONFIG_DIR` survive later publication. Pi settings, credentials, model catalogs, Pi Packages, and sessions are ordinary Pi-owned files in the stable `$PI_CODING_AGENT_DIR`; a new Environment does not copy the original `~/.pi/agent` state. Woma strips and regenerates its marked Codex MCP blocks and exact Claude Hook entries while preserving user-owned provider settings. Claude `.claude.json` remains in the stable home, where Woma transactionally updates only Package-managed `mcpServers`. Pi has no Woma MCP or Hook adapter, so Package validation rejects those resources when they target Pi instead of silently omitting them. Qoder MCP servers and Hooks live directly in the managed `settings.json` view: Woma removes and regenerates exactly the Package-managed `mcpServers` and Hook entries while preserving user-owned settings edited through `$QODER_CONFIG_DIR/settings.json`. Woma never seeds or captures Qoder credentials; login state is ordinary Qoder-owned opaque data in the stable Environment home.

Every other path is opaque Agent-owned state. Environment-local Skill inventory enumerates only direct, non-hidden children of the shared Skill root and reads only their `SKILL.md` frontmatter; it does not hash, copy, adopt, or package them. Apart from that read-only inventory and the one-time merge of legacy Agent Skill roots, Environment initialization and view publication do not inspect unknown files, including SQLite main, WAL, and SHM files. The compatibility move stays within one Environment, preserves unique entries without reading their contents, and reads complete trees only when needed to resolve a same-name collision. Equivalent duplicates are staged until publication succeeds, and all moves participate in publication rollback. `woma migrate sessions` is a separate explicit operation over documented session and history paths: it builds and verifies a link-free temporary snapshot, preflights target conflicts, structurally merges JSONL records, then publishes ordinary files into the selected stable home under the Environment lock. Structured files are atomically replaced and restored on ordinary publication failure. Retired generations contain only managed resources and remain available to processes that still have those files open.

`woma init` follows Conda's shell-initialization user model while adapting it to npm's movable installation paths. It writes the rendered hook to the stable `$WOMA_HOME/shell` directory and maintains a marker-delimited source block in the selected shell profile. Ordinary shell startup therefore reads a static script and never launches Node. Initialization is idempotent, preserves profile modes and symbolic links, supports dry runs and reversal, and never initializes an Environment.

The static hook generated by `woma init` performs no Woma-state or project filesystem operations and acquires no locks. The sourced hook saves the original Agent configuration roots and wraps the installed `woma` CLI. It uses bounded filesystem checks to select an existing Environment with a complete view, falls back to an existing `base` for a stale inherited selection, and restores every original Agent home with no selected Environment when `base` is unavailable. It parses only a real top-level `activate` or `deactivate` command. A successful `deactivate` clears the shell selection and restores the original Agent homes; selecting Woma `base` remains the explicit `woma activate base` operation. Help requests and unrelated arguments never change Environment state. For unsupported targets it restores the original Agent configuration root. It never proxies `codex`, `claude`, `pi`, or `qodercli`. Explicit commands that require `base` retain lazy initialization and transactional repair.

Environment schema v2 removes the two roots that schema v1 inserted automatically. The migration prunes the exact implicit `woma-project-memory` and `woma-package-builder` roots while retaining Packages still reachable from an explicit root; a retained dependency on the legacy implicit Memory Package fails with an actionable error. Activation also removes only exact, unmodified Woma Project Memory discovery blocks left in `AGENTS.md` or `CLAUDE.md`. It preserves surrounding content, symbolic links, and file modes, fails closed on modified blocks, and never reads, writes, or deletes Memory data. No discovery block is ever added; this compatibility cleanup is the only project-context mutation.

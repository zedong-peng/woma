...
An ordinary Skill written inside an already selected Codex Environment immediately belongs to that Environment, just as a pip-installed package belongs to its active Conda environment. Harness discovers non-hidden direct children with valid `SKILL.md` metadata during `list`, `info`, and `doctor`, labels their origin `external`, and never infers which tool created them. These Environment-local Skills are not Harness Packages: they do not enter the recipe, lock, Package Store, or bundle, and other Environments remain unchanged. Hidden paths such as Codex-owned `.system` are excluded. Harness does not watch the directory continuously or proxy the Codex process because Codex already reads the same stable Environment home directly.
...
The manual shell-hook executable path imports only the deterministic hook renderer; it does not load Commander or the Environment, Package, validation, synchronization, view, Memory, or project-mutation modules. Hook generation performs no Harness-state or project filesystem operations and acquires no locks. The sourced hook saves the original Agent configuration roots and wraps both installed CLI names, `harness` and `harness-conda`. It uses bounded filesystem checks to select an existing Environment with a complete view, falls back to an existing `base` for a stale inherited selection, and restores every original Agent home with no selected Environment when `base` is unavailable. It parses only a real top-level `activate` or `deactivate` command. Help requests and unrelated arguments never change Environment state. For unsupported targets it restores the original Agent configuration root. It never proxies `codex`, `claude`, or `pi`. Explicit commands that require `base` retain lazy initialization and transactional repair.
...
Environment mutations are serialized with filesystem-backed per-Environment locks. Activation holds the target lock through the project transition, preventing concurrent install or uninstall from invalidating the selected generation. Project locks are keyed by canonical real paths so symlink aliases serialize together. Recipes and locks are re-read inside the lock, and multi-file inspection uses the same locked snapshot. Package cache publication, reads, permission changes, corruption repair, and verification use per-Package locks. Package resolution and validation complete before one complete view generation is atomically published. Install and uninstall repair the current lock first and share the same publication path: recipe, lock, stable managed-home links, Package-managed Claude MCP fields, and the complete view pointer remain inside the ordinary-error rollback boundary. Environment-local Skills and other opaque Agent state are outside the transaction because Harness never mutates them. Abrupt process termination and unanticipated filesystem failure remain outside the guarantee because there is no persistent transaction journal.

...
| Machine-specific Memory | Project-local, project-managed | `<project>/.harness/local/` |
...
Project Memory is project-owned context outside the Environment model. The `harness-project-memory` Package is available through an Environment view only when explicitly installed, and its Skill runs only when an Agent or user explicitly selects it. Harness does not initialize Memory files, change project ignore rules, or add discovery pointers to `AGENTS.md` or `CLAUDE.md`. The optional Skill can call `harness info --json` to locate project paths and map selected Skills to Packages, then read relevant Memory as user-requested context. Exact marker blocks written by earlier releases are removed once during legacy cleanup; surrounding content, symbolic links, and file modes are preserved.

...
description: Loads and maintains durable project-specific knowledge for active Harness packages and Skills. Use at the beginning of every Agent session in a Harness Environment, before using another active Skill, and whenever the user states stable project facts such as build, test, benchmark, repository, or operational conventions, even when the user does not explicitly ask to remember them.
---
...
1. Run `harness current --json` from the current project or a nested directory.
...
Do not require third-party Skills to know about Harness Memory. Apply the relevant Memory before following their instructions.
...
- Write knowledge shared by all project work to the `memory.project` path returned by `harness current --json`.
